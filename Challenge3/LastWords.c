...
int check_code(char *input) {
    // Correct password "VOID{H1DD3N_C0D3}" obfuscated with XOR 0x42
    char correct_code[] = {0x14, 0xd, 0xb, 0x6, 0x39, 0xa, 0x73, 0x6, 0x6, 0x71, 0xc, 0x1d, 0x1, 0x72, 0x6, 0x71, 0x3f, 0x00};
...
int main() {
    char input[18] = {0}; // Initialize to 0 to ensure null-termination

    display_banner(); // Display the banner and the lore

    printf("Enter the activation code: ");
    scanf("%17s", input); // Limit to 14 characters to prevent overflow
